...
inference_files = [
    ("adversarial", "full_adversarial.json"),
    ("dilemma",      "full_dilemma.json")
]
...
for prefix, filepath in inference_files:
    ...
    for idx, entry in enumerate(entries, start=1):
        ...
        for model_id, label in [
            ...
            print(f"    ↳ [{label}] Sending to model '{model_id}'...", end="", flush=True)
            ...
            # ▼ Changed: Use output_text instead of choices
            result_text = response.output_text  # :contentReference[oaicite:0]{index=0}

            # Generate filename
            fn = f"logs/{prefix}_{idx:03d}_{label}.txt"
